<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>index</title>
	</head>
	<body>
		<h3>10장.MVC 실습</h3>
		
		<h4>MVC 기본</h4>
		<a href="/Ch10/hello.do">hello</a>
		<a href="/Ch10/welcome.do">welcome</a>
		<a href="/Ch10/greeting.do">greeting</a>
		
		<h4>User1 실습</h4>
		<a href="/Ch10/user1/list.do">목록</a>
		<a href="/Ch10/user1/.do">등록</a>
		
		<h4>User2 실습</h4>
		<a href="/Ch10/user2/list.do">목록</a>
		<a href="/Ch10/user2/.do">등록</a>
		
		<h4>User3 실습</h4>
		<a href="/Ch10/user3/list.do">목록</a>
		<a href="/Ch10/user3/.do">등록</a>
		
		<h4>User4 실습</h4>
		<a href="/Ch10/user4/list.do">목록</a>
		<a href="/Ch10/user4/.do">등록</a>
		
		<h4>User5 실습</h4>
		<a href="/Ch10/user5/list.do">목록</a>
		<a href="/Ch10/user5/.do">등록</a>
		
		<h4>User6 실습</h4>
		<a href="/Ch10/user6/list.do">목록</a>
		<a href="/Ch10/user6/.do">등록</a>
		
	</body>
</html>